...
class JavaPlanBuilder(wayangCtx: WayangContext, jobName: String) {
  ...
  def readFromRestAPI(apiURL: String, apiMethod: String = "GET", headers: String = ""): UnarySourceDataQuantaBuilder[UnarySourceDataQuantaBuilder[_, String], String] =
   createSourceBuilder(new RestAPISource(apiURL, apiMethod, headers))(ClassTag(classOf[String]))
